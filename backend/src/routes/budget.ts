...
// zurückkehren.
const selectBudgetOwnerByIdStmt = db.prepare('SELECT owner_id FROM budgets WHERE id = ?');
const selectBudgetItemsByTripStmt = db.prepare(
...
const insertBudgetItemStmt = db.prepare(
  `INSERT INTO budget_items (trip_id, title, category, amount, paid_by_user_id, date, note, budget_id)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
);
const selectBudgetItemByIdStmt = db.prepare('SELECT * FROM budget_items WHERE id = ?');
const selectBudgetItemAuthStmt = db.prepare(
  'SELECT trip_id, budget_id FROM budget_items WHERE id = ?'
);
const updateBudgetItemStmt = db.prepare(
...
const selectBudgetsByTripStmt = db.prepare(
...
const insertBudgetStmt = db.prepare(
  'INSERT INTO budgets (trip_id, name, owner_id, target_amount) VALUES (?, ?, ?, ?)'
);
const selectBudgetByIdStmt = db.prepare('SELECT * FROM budgets WHERE id = ?');
const selectBudgetAuthStmt = db.prepare('SELECT trip_id, owner_id FROM budgets WHERE id = ?');
const updateBudgetStmt = db.prepare(
  'UPDATE budgets SET name = ?, owner_id = ?, target_amount = ? WHERE id = ?'
);
const deleteBudgetStmt = db.prepare('DELETE FROM budgets WHERE id = ?');
const selectAllocationsByTripStmt = db.prepare(
  `SELECT a.* FROM budget_allocations a
   JOIN budgets b ON b.id = a.budget_id
   WHERE b.trip_id = ? AND (b.owner_id IS NULL OR b.owner_id = ?)
   ORDER BY a.category`
);
const upsertAllocationStmt = db.prepare(
...
const selectAllocationByCategoryStmt = db.prepare(
  'SELECT * FROM budget_allocations WHERE budget_id = ? AND category = ?'
);
const selectAllocationAuthStmt = db.prepare(
...
const deleteAllocationStmt = db.prepare('DELETE FROM budget_allocations WHERE id = ?');
const selectTransfersByTripStmt = db.prepare(
...
const selectTransferByIdStmt = db.prepare('SELECT * FROM budget_transfers WHERE id = ?');
const selectTransferTripIdStmt = db.prepare('SELECT trip_id FROM budget_transfers WHERE id = ?');
const updateTransferStmt = db.prepare(
  'UPDATE budget_transfers SET from_user_id = ?, to_user_id = ?, amount = ?, date = ?, note = ? WHERE id = ?'
);
const softDeleteTransferStmt = db.prepare(
  'UPDATE budget_transfers SET deleted_at = ? WHERE id = ? AND deleted_at IS NULL'
);
...
function requireBudgetAccessIfLinked(
  ...
  const budget = selectBudgetOwnerByIdStmt.get(budgetId) as { owner_id: number | null } | undefined;
  if (!budget) {
...
export const budgetRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get<{ Querystring: { trip_id?: string } }>('/budget', async (req, reply) => {
    ...
    return selectBudgetItemsByTripStmt.all(req.query.trip_id, req.session.userId ?? null);
  });
  ...
  app.post<{ Body: ExpenseBody }>('/budget', async (req, reply) => {
    ...
    reply.code(201);
    return selectBudgetItemByIdStmt.get(result.lastInsertRowid);
  });

  app.put<{ Params: { id: string }; Body: ExpenseBody }>('/budget/:id', async (req, reply) => {
    const existingExpense = selectBudgetItemAuthStmt.get(req.params.id) as
      { trip_id: number; budget_id: number | null } | undefined;
    ...
    return selectBudgetItemByIdStmt.get(req.params.id);
  });
  ...
  app.delete<{ Params: { id: string } }>('/budget/:id', async (req, reply) => {
    ...
    const result = softDeleteBudgetItemStmt.run(new Date().toISOString(), req.params.id);
    if (result.changes === 0) return reply.code(404).send({ error: 'Nicht gefunden' });
    recordActivity(
      existingExpense.trip_id,
      'budget',
  ...
  app.get<{ Querystring: { trip_id?: string } }>('/budget/budgets', async (req, reply) => {
    if (!requireTripId(req.query, reply)) return;
    if (!requireTripMember(reply, req.query.trip_id, req.session.userId)) return;
    return selectBudgetsByTripStmt.all(req.query.trip_id, req.session.userId ?? null);
  });
  ...
  app.post<{ Body: BudgetBody }>('/budget/budgets', async (req, reply) => {
    ...
    const result = insertBudgetStmt.run(
      trip_id,
      name.trim(),
      owner_id ?? null,
      target_amount ?? null
    ...
    reply.code(201);
    return selectBudgetByIdStmt.get(result.lastInsertRowid);
  });

  app.put<{ Params: { id: string }; Body: BudgetBody }>(
    '/budget/budgets/:id',
    async (req, reply) => {
      const existingBudget = selectBudgetAuthStmt.get(req.params.id) as
        { trip_id: number; owner_id: number | null } | undefined;
      ...
      const result = updateBudgetStmt.run(
        name.trim(),
        owner_id ?? null,
        target_amount ?? null,
        req.params.id
      ...
      return selectBudgetByIdStmt.get(req.params.id);
    }
  );
  ...
  app.delete<{ Params: { id: string } }>('/budget/budgets/:id', async (req, reply) => {
    ...
    const result = deleteBudgetStmt.run(req.params.id);
    if (result.changes === 0) return reply.code(404).send({ error: 'Nicht gefunden' });
    recordActivity(
      existingBudget.trip_id,
      'budget',
  ...
  app.get<{ Querystring: { trip_id?: string } }>('/budget/allocations', async (req, reply) => {
    if (!requireTripId(req.query, reply)) return;
    if (!requireTripMember(reply, req.query.trip_id, req.session.userId)) return;
    return selectAllocationsByTripStmt.all(req.query.trip_id, req.session.userId ?? null);
  });

  app.put<{ Body: AllocationBody }>('/budget/allocations', async (req, reply) => {
    const { budget_id, category, amount } = req.body;
    const parentBudget = selectBudgetAuthStmt.get(budget_id) as
      { trip_id: number; owner_id: number | null } | undefined;
  ...
  app.delete<{ Params: { id: string } }>('/budget/allocations/:id', async (req, reply) => {
    const allocation = selectAllocationAuthStmt.get(req.params.id) as
      { id: number; trip_id: number; owner_id: number | null } | undefined;
    ...
    const result = deleteAllocationStmt.run(req.params.id);
    if (result.changes === 0) return reply.code(404).send({ error: 'Nicht gefunden' });
    return reply.code(204).send();
  });
  ...
  app.get<{ Querystring: { trip_id?: string } }>('/budget/transfers', async (req, reply) => {
    if (!requireTripId(req.query, reply)) return;
    if (!requireTripMember(reply, req.query.trip_id, req.session.userId)) return;
    return selectTransfersByTripStmt.all(req.query.trip_id);
  });
  ...
  app.post<{ Body: TransferBody }>('/budget/transfers', async (req, reply) => {
    ...
    const result = insertTransferStmt.run(
      trip_id,
      from_user_id,
      to_user_id,
      amount,
    ...
    reply.code(201);
    return selectTransferByIdStmt.get(result.lastInsertRowid);
  });

  app.put<{ Params: { id: string }; Body: TransferBody }>(
    '/budget/transfers/:id',
    async (req, reply) => {
      const existingTransfer = selectTransferTripIdStmt.get(req.params.id) as
      ...
      const result = updateTransferStmt.run(
        from_user_id,
        to_user_id,
        amount,
        date ?? null,
      ...
      return selectTransferByIdStmt.get(req.params.id);
    }
  );
  ...
  app.delete<{ Params: { id: string } }>('/budget/transfers/:id', async (req, reply) => {
    ...
    const result = softDeleteTransferStmt.run(new Date().toISOString(), req.params.id);
    if (result.changes === 0) return reply.code(404).send({ error: 'Nicht gefunden' });
    recordActivity(
      existingTransfer.trip_id,
      'budget',
