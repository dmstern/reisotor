...
function syncExcursionLegs(
  ...
  if (legs && legs.length > 0) {
    for (let index = 0; index < legs.length; index++) {
      ...
      if (hasAmount) {
        const fromTitle =
          (selectSpotTitleByIdForLegsStmt.get(leg.from_spot_id) as { title: string } | undefined)
            ?.title ?? '';
        const toTitle =
          (selectSpotTitleByIdForLegsStmt.get(leg.to_spot_id) as { title: string } | undefined)
            ?.title ?? '';
