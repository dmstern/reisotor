...
function isTrackVisible(track: Pick<TrackRow, 'user_id' | 'visibility'>, userId: number | undefined): boolean {
...
export const tracksRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: StartTrackBody }>('/tracks', async (req, reply) => {
    ...
    reply.code(201);
    return db.prepare('SELECT * FROM location_tracks WHERE id = ?').get(result.lastInsertRowid) as TrackRow;
  });
  ...
  app.post<{ Params: { id: string }; Body: AppendPointsBody }>('/tracks/:id/points', async (req, reply) => {
  ...
  app.put<{ Params: { id: string }; Body: UpdateTrackBody }>('/tracks/:id', async (req, reply) => {
    ...
    db.prepare('UPDATE location_tracks SET title = ?, visibility = ?, excursion_id = ? WHERE id = ?').run(
      title,
      visibility,
      excursionId,
      track.id,
