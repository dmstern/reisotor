...
export const notificationsRoutes: FastifyPluginAsync = async (app) => {
  app.get<{ Querystring: { trip_id?: string } }>('/notifications', async (req, reply) => {
    if (!requireTripMember(reply, req.query.trip_id, req.session.userId)) return;
    const rows = db
      .prepare(
        `SELECT trip_activity.*, users.username AS actor_username, users.avatar AS actor_avatar,
  ...
  app.post<{ Body: { trip_id?: number } }>('/notifications/read-all', async (req, reply) => {
    const { trip_id } = req.body ?? {};
    if (!requireTripMember(reply, trip_id, req.session.userId)) return;
    const now = new Date().toISOString();
    db.prepare(
      `INSERT INTO notification_reads (user_id, activity_id, read_at)
