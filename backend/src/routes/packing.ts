...
export function clampCounts(
  quantityRaw: number | undefined,
  laidOutRaw: number | undefined,
  packedRaw: number | undefined
) {
...
function categoryRequired(tripId: number): boolean {
  const trip = db
    ...
    .get(tripId) as { packing_category_required: number } | undefined;
...
export const packingRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post<{ Body: PackingBody }>('/packing', async (req, reply) => {
    ...
    const { quantity, laidOut, packed } = clampCounts(
      req.body.quantity,
      req.body.laid_out_count,
      req.body.packed_count
    );
    const result = db
      .prepare(
  ...
  app.put<{ Params: { id: string }; Body: PackingBody }>('/packing/:id', async (req, reply) => {
    const existingItem = db
      ...
      .get(req.params.id) as { trip_id: number; category: string | null } | undefined;
