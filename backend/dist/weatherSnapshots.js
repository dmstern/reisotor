...
async function snapshotTripWeather(trip, today) {
    ...
    if (trip.start_date > yesterday)
        return; // noch kein abgeschlossener Urlaubstag
    ...
    const existing = db
        ...
        .all(trip.id, trip.start_date, rangeEnd);
    const existingDates = new Set(existing.map((r) => r.date));
    const missingDates = wantedDates.filter((d) => !existingDates.has(d));
    if (!missingDates.length)
        return; // kein Fetch, wenn ohnehin schon alles gespeichert ist
    ...
    data.daily.time.forEach((date, i) => {
        if (!missingSet.has(date))
            return;
        upsertSnapshot.run(trip.id, date, data.daily.weathercode[i], data.daily.temperature_2m_max[i], data.daily.temperature_2m_min[i], data.daily.precipitation_probability_max?.[i] ?? null);
    });
}
...
export async function refreshTripWeatherSnapshots(tripId) {
    const trip = db.prepare('SELECT id, lat, lng, start_date, end_date FROM trips WHERE id = ?').get(tripId);
