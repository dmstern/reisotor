...
if (!hadTripMembersTable) {
    const allTripsForMembershipBackfill = db.prepare('SELECT id FROM trips').all();
    const allUsersForMembershipBackfill = db.prepare('SELECT id FROM users').all();
    const insertBackfillMembership = db.prepare('INSERT OR IGNORE INTO trip_members (trip_id, user_id, created_at) VALUES (?, ?, ?)');
...
if (hasColumn('packing_items', 'checked')) {
    db.exec(`UPDATE packing_items SET packed_count = quantity, laid_out_count = quantity
...
const firstTrip = db.prepare('SELECT id FROM trips ORDER BY id LIMIT 1').get();
if (firstTrip) {
    for (const table of TRIP_SCOPED_TABLES) {
...
export function ensureDefaultSharedBudget(tripId) {
    const existing = db
        .prepare('SELECT id FROM budgets WHERE trip_id = ? AND owner_id IS NULL ORDER BY id LIMIT 1')
        .get(tripId);
    const budgetId = existing?.id ??
        db.prepare('INSERT INTO budgets (trip_id, name, owner_id) VALUES (?, ?, NULL)').run(tripId, 'Gemeinsames Budget')
            .lastInsertRowid;
    const insertAllocation = db.prepare('INSERT OR IGNORE INTO budget_allocations (budget_id, category, amount) VALUES (?, ?, 0)');
...
if (hasTable('budget_targets')) {
    const migrateBudgets = db.transaction(() => {
        const targets = db.prepare('SELECT * FROM budget_targets').all();
        const categoryTargets = hasTable('budget_category_targets')
            ? db.prepare('SELECT * FROM budget_category_targets').all()
            : [];
        ...
        const insertAllocation = db.prepare(`INSERT INTO budget_allocations (budget_id, category, amount) VALUES (?, ?, ?)
       ON CONFLICT(budget_id, category) DO UPDATE SET amount = amount + excluded.amount`);
        const usernameOf = (id) => db.prepare('SELECT username FROM users WHERE id = ?').get(id)
            ?.username ?? `Nutzer ${id}`;
        for (const target of targets) {
            if (target.owner_id === null) {
                ...
                const budgetId = insertBudget.run(target.trip_id, 'Gemeinsames Budget', null).lastInsertRowid;
                ...
                const budgetId = insertBudget.run(target.trip_id, `Persönliches Budget (${usernameOf(target.owner_id)})`, target.owner_id).lastInsertRowid;
...
const allTrips = db.prepare('SELECT id FROM trips').all();
const sharedBudgetIdByTrip = new Map();
...
for (const row of orphanedAutoExpenses) {
    ...
    if (budgetId)
        linkAutoExpense.run(budgetId, row.id);
}
...
if (hasTable('travel_places')) {
    const places = db.prepare('SELECT * FROM travel_places').all();
    const placeToSpotId = new Map();
    const insertSpot = db.prepare(`INSERT INTO spots (trip_id, title, category, maps_link, lat, lng, is_home) VALUES (?, ?, ?, ?, ?, ?, ?)`);
    for (const p of places) {
        const result = insertSpot.run(p.trip_id, p.name, p.type, p.maps_link, p.lat, p.lng, p.is_home);
        placeToSpotId.set(p.id, result.lastInsertRowid);
    }
  ...
  `);
    const oldItems = db.prepare('SELECT * FROM travel_items_migrating').all();
  ...
  `);
    for (const row of oldItems) {
        const fromPlaceId = row.from_place_id != null ? placeToSpotId.get(row.from_place_id) ?? null : null;
        const toPlaceId = row.to_place_id != null ? placeToSpotId.get(row.to_place_id) ?? null : null;
        insertItem.run(row.id, row.title, row.type, row.from_location, row.to_location, row.date, row.departure_time, row.checkin_info, row.amount, row.paid_by_user_id, row.luggage, row.seat, row.link, row.note, row.budget_expense_id, row.from_maps_link, row.from_lat, row.from_lng, row.to_maps_link, row.to_lat, row.to_lng, row.role, row.arrival_time, fromPlaceId, toPlaceId, row.trip_id, row.deleted_at);
    }
    ...
    if (hasColumn('excursion_spots', 'station_key')) {
        ...
        for (const row of legacyStationRows) {
            ...
            if (newSpotId != null)
                updateStationKey.run(`spot-${newSpotId}`, row.id);
        }
    }
...
if (hasTable('accommodation')) {
    const accommodations = db.prepare('SELECT * FROM accommodation').all();
    const accommodationToSpotId = new Map();
    ...
    `);
        for (const a of accommodations) {
            const result = insertAccommodationSpot.run(a.trip_id, a.name, a.note, a.maps_link, a.lat, a.lng, a.address, a.start_date, a.end_date, a.checkin, a.checkout, a.contact, a.amount, a.paid_by_user_id, a.budget_expense_id, a.deleted_at);
            accommodationToSpotId.set(a.id, result.lastInsertRowid);
        }
    }
    ...
    if (hasColumn('excursion_spots', 'station_key')) {
        ...
        for (const row of legacyAccommodationStationRows) {
            ...
            if (newSpotId != null)
                updateAccommodationStationKey.run(`spot-${newSpotId}`, row.id);
        }
    }
    ...
    for (const row of legacyAccommodationAttachmentRows) {
        ...
        if (newSpotId != null)
            updateAttachmentDomain.run(newSpotId, row.id);
    }
...
if (hasColumn('excursion_spots', 'station_key')) {
    const legacyRows = db.prepare('SELECT * FROM excursion_spots').all();
    ...
    function resolveLegacySpotId(stationKey) {
  ...
  `);
    if (legacyRows.length > 0) {
        ...
        // Zweig ohnehin nie.
        const insertSpotLink = db.prepare('INSERT INTO excursion_spots (idea_id, spot_id, position) VALUES (?, ?, ?)');
        ...
        for (const row of legacyRows) {
            ...
            if (spotId == null || !spotExists.get(spotId))
                continue;
...
const ATTACHMENT_DOMAIN_BY_TABLE = {
...
if (hasTable('travel_items')) {
    ...
    const allRows = db.prepare('SELECT * FROM travel_items').all();
    ...
    `);
        const insertSpot = db.prepare('INSERT INTO spots (trip_id, title, maps_link, lat, lng) VALUES (?, ?, ?, ?, ?)');
        ...
        const insertSchedule = db.prepare('INSERT INTO schedule_items (trip_id, date, title, idea_id) VALUES (?, ?, ?, ?)');
        ...
        for (const row of allRows) {
            ...
            if (ideaId == null) {
                ideaId = insertIdea.run(row.title, row.note, row.note_format ?? 'legacy', row.trip_id, row.role, row.type, row.departure_time, row.arrival_time, row.checkin_info, row.amount, row.paid_by_user_id, row.luggage, row.seat, row.link, row.deleted_at).lastInsertRowid;
                let position = 0;
                const addStation = (placeId, location, mapsLink, lat, lng) => {
                    const spotId = placeId ?? (location ? insertSpot.run(row.trip_id, location, mapsLink, lat, lng).lastInsertRowid : null);
                    if (spotId == null)
                        return;
            ...
            if (row.budget_expense_id) {
                const idea = getIdeaBudgetExpense.get(ideaId);
        ...
        if (travelAttachments.length > 0) {
            ...
            for (const a of travelAttachments) {
                const ti = findMigratedIdea.get(a.entity_id);
                if (ti?.migrated_idea_id != null)
                    moveAttachment.run(ti.migrated_idea_id, a.id);
            }
        }
    }
