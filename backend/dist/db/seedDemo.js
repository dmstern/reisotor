...
const insertUser = db.prepare('INSERT OR IGNORE INTO users (username, password_hash, avatar, is_admin, must_change_password) VALUES (?, ?, ?, ?, ?)');
for (let i = 0; i < users.length; i++) {
    const u = users[i];
    insertUser.run(u.username, bcrypt.hashSync(u.password, 10), u.avatar, i === 0 ? 1 : 0, 0);
}
const [user1, user2] = users.map((u) => db.prepare('SELECT id FROM users WHERE username = ?').get(u.username));
const today = new Date();
const startDate = addDays(today, 14);
const endDate = addDays(today, 24);
const LISBON = { lat: 38.7223, lng: -9.1393 };
const tripResult = db
    .prepare(`INSERT INTO trips (name, destination, start_date, end_date, maps_link, lat, lng, image_url)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
    .run('Sommerurlaub Lissabon', 'Lissabon, Portugal', fmt(startDate), fmt(endDate), null, LISBON.lat, LISBON.lng, tilePreviewUrl(LISBON.lat, LISBON.lng));
...
const insertMembership = db.prepare('INSERT OR IGNORE INTO trip_members (trip_id, user_id, created_at) VALUES (?, ?, ?)');
...
const updateAllocation = db.prepare('UPDATE budget_allocations SET amount = ? WHERE budget_id = ? AND category = ?');
for (const [category, amount] of Object.entries(allocations)) {
    updateAllocation.run(amount, sharedBudgetId, category);
}
const insertExpense = db.prepare(`INSERT INTO budget_items (trip_id, title, category, amount, paid_by_user_id, date, note, budget_id)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
...
const accommodationExpenseId = insertExpense.run(tripId, 'Hotel Alfama', 'Unterkunft', accommodationAmount, user1.id, fmt(startDate), 'Automatisch aus Unterkunft-Eintrag', sharedBudgetId).lastInsertRowid;
db.prepare(`INSERT INTO spots
   ...
   VALUES (?, ?, 'Unterkunft', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(tripId, 'Hotel Alfama', 'Zentrale Lage im Altstadtviertel Alfama, Klimaanlage vorhanden.', null, LISBON.lat + 0.0015, LISBON.lng + 0.001, 'Rua de São Pedro 12, 1100-590 Lisboa', fmt(startDate), fmt(endDate), '15:00', '11:00', 'reservas@hotelalfama.example', accommodationAmount, user1.id, accommodationExpenseId);
...
db.prepare('INSERT INTO budget_transfers (trip_id, from_user_id, to_user_id, amount, date, note) VALUES (?, ?, ?, ?, ?, ?)').run(tripId, user2.id, user1.id, 120, fmt(addDays(startDate, 4)), 'Teilausgleich für Hotel');
// --- Ablauf/Kalender ---
const insertSchedule = db.prepare(`INSERT INTO schedule_items (trip_id, date, end_date, time, title, note, location, category)
   VALUES (?, ?, ?, ?, ?, ?, ?, 'other')`);
...
const insertPacking = db.prepare('INSERT INTO packing_items (trip_id, category, label, owner_id, packed_count, laid_out_count) VALUES (?, ?, ?, ?, ?, ?)');
...
// --- Einkaufsliste ---
const insertShopping = db.prepare('INSERT INTO shopping_items (trip_id, label, assigned_to_user_id, checked, link, note, shop, period) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
...
const insertSpot = db.prepare(`INSERT INTO spots (trip_id, title, image_url, category, note, maps_link, lat, lng, created_by)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
const belemLat = 38.6916;
const belemLng = -9.2159;
const belemSpotId = insertSpot.run(tripId, 'Torre de Belém', tilePreviewUrl(belemLat, belemLng), 'Sehenswürdigkeit', 'Ikonischer Wehrturm direkt am Tejo.', null, belemLat, belemLng, user1.id).lastInsertRowid;
const marketLat = 38.7075;
const marketLng = -9.1459;
const marketSpotId = insertSpot.run(tripId, 'Time Out Market', tilePreviewUrl(marketLat, marketLng), 'Restaurant', 'Große Markthalle mit vielen Ständen bekannter Lissabonner Restaurants.', null, marketLat, marketLng, user2.id).lastInsertRowid;
db.prepare('INSERT INTO spot_likes (spot_id, user_id, created_at) VALUES (?, ?, ?)').run(belemSpotId, user2.id, new Date().toISOString());
db.prepare('INSERT INTO spot_comments (spot_id, author_id, content, created_at) VALUES (?, ?, ?, ?)').run(belemSpotId, user2.id, 'Unbedingt früh morgens hin, bevor die Reisebusse kommen!', new Date().toISOString());
...
const insertHomeSpot = db.prepare('INSERT INTO spots (trip_id, title, category, is_home) VALUES (?, ?, ?, 1)');
...
const lisbonAirportSpotId = insertSpot.run(tripId, 'Lissabon (LIS)', null, 'Flughafen', null, null, LISBON_AIRPORT.lat, LISBON_AIRPORT.lng, user1.id).lastInsertRowid;
const insertTravelIdea = db.prepare(`INSERT INTO ideas (
...
const insertTravelSchedule = db.prepare('INSERT INTO schedule_items (trip_id, date, title, idea_id) VALUES (?, ?, ?, ?)');
const outboundExpenseId = insertExpense.run(tripId, 'Hinflug nach Lissabon', 'Transport', 189, user2.id, fmt(startDate), 'Automatisch aus Tour', sharedBudgetId).lastInsertRowid;
const outboundIdeaId = insertTravelIdea.run(tripId, 'Hinflug nach Lissabon', 'Direktflug', user2.id, 'arrival', 'Flug', '07:20', '10:05', 'Online-Check-in ab 24h vorher', 189, user2.id, '1x 23kg Koffer', '14C', outboundExpenseId).lastInsertRowid;
...
const returnExpenseId = insertExpense.run(tripId, 'Rückflug nach Berlin', 'Transport', 189, user1.id, fmt(endDate), 'Automatisch aus Tour', sharedBudgetId).lastInsertRowid;
const returnIdeaId = insertTravelIdea.run(tripId, 'Rückflug nach Berlin', 'Direktflug', user1.id, 'departure', 'Flug', '18:40', '22:15', null, 189, user1.id, '1x 23kg Koffer', '9A', returnExpenseId).lastInsertRowid;
...
db.prepare('INSERT INTO schedule_items (trip_id, date, title, idea_id) VALUES (?, ?, ?, ?)').run(tripId, fmt(addDays(startDate, 2)), 'Sightseeing-Tag Belém', ideaId);
db.prepare('INSERT INTO idea_likes (idea_id, user_id, created_at) VALUES (?, ?, ?)').run(ideaId, user2.id, new Date().toISOString());
db.prepare('INSERT INTO idea_comments (idea_id, author_id, content, created_at) VALUES (?, ?, ?, ?)').run(ideaId, user2.id, 'Klingt gut, sollten wir früh starten!', new Date().toISOString());
// --- Tagebuch ---
const diaryResult = db
    .prepare('INSERT INTO diary_entries (trip_id, author_id, title, content, images, created_at) VALUES (?, ?, ?, ?, ?, ?)')
    .run(tripId, user1.id, 'Ankunft in Lissabon', 'Nach dem Flug direkt ins Hotel und dann noch einen Abendspaziergang durch die Alfama gemacht. Traumhafter Blick vom Miradouro!', JSON.stringify([]), new Date().toISOString());
const diaryEntryId = diaryResult.lastInsertRowid;
db.prepare('INSERT INTO diary_likes (entry_id, user_id, created_at) VALUES (?, ?, ?)').run(diaryEntryId, user2.id, new Date().toISOString());
db.prepare('INSERT INTO diary_comments (entry_id, author_id, content, created_at) VALUES (?, ?, ?, ?)').run(diaryEntryId, user2.id, 'Sieht traumhaft aus, ich freu mich schon!', new Date().toISOString());
// --- Notizen ---
db.prepare('INSERT INTO notes (trip_id, title, content, created_by, created_at) VALUES (?, ?, ?, ?, ?)').run(tripId, 'WLAN & Notfallkontakte', 'Hotel-WLAN: siehe Zimmerkarte. Notfallnummer Portugal: 112.', user1.id, new Date().toISOString());
console.log(`Demo-Seed abgeschlossen: Trip "${tripId}" mit Unterkunft, Reise, Kalender, Packliste, Einkaufsliste, Ausflug/Spots, Budget, Tagebuch und Notiz angelegt.`);
