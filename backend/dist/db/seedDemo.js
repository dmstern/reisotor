...
const insertTravel = db.prepare(`INSERT INTO travel_items
   ...
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
const outboundExpenseId = insertExpense.run(tripId, 'Hinflug nach Lissabon', 'Transport', 189, user2.id, fmt(startDate), 'Automatisch aus Reise-Eintrag', sharedBudgetId).lastInsertRowid;
insertTravel.run(tripId, 'Hinflug nach Lissabon', 'flight', 'Berlin (BER)', 'Lissabon (LIS)', fmt(startDate), '07:20', '10:05', 'Online-Check-in ab 24h vorher', 189, user2.id, '1x 23kg Koffer', '14C', 'Direktflug', outboundExpenseId, 'arrival');
const returnExpenseId = insertExpense.run(tripId, 'Rückflug nach Berlin', 'Transport', 189, user1.id, fmt(endDate), 'Automatisch aus Reise-Eintrag', sharedBudgetId).lastInsertRowid;
insertTravel.run(tripId, 'Rückflug nach Berlin', 'flight', 'Lissabon (LIS)', 'Berlin (BER)', fmt(endDate), '18:40', '22:15', null, 189, user1.id, '1x 23kg Koffer', '9A', 'Direktflug', returnExpenseId, 'departure');
