...
export async function fetchRegionInfo(countryCode, homeCurrency) {
    ...
    const [countryResult, advisoryResult] = await Promise.allSettled([
        fetch(`https://restcountries.com/v3.1/alpha/${countryCode}`, { signal: AbortSignal.timeout(5000) }).then((res) => res.ok ? res.json() : Promise.reject(new Error(String(res.status)))),
        fetch(`https://www.travel-advisory.info/api?countrycode=${countryCode}`, {
            signal: AbortSignal.timeout(5000),
        }).then((res) => (res.ok ? res.json() : Promise.reject(new Error(String(res.status))))),
    ]);
    const languages = countryResult.status === 'fulfilled' ? Object.values(countryResult.value[0]?.languages ?? {}) : [];
    const currencyEntry = countryResult.status === 'fulfilled' ? Object.entries(countryResult.value[0]?.currencies ?? {})[0] : undefined;
