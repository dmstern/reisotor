...
export const REGISTRATION_MODE = parseMode(process.env.REGISTRATION_MODE);
...
const FULL_ACCESS_USERS = new Set((process.env.REGISTRATION_FULL_ACCESS_USERS ?? '')
    .split(',')
    .map((name) => name.trim())
    .filter(Boolean));
...
export function isUserRestricted(userId) {
    if (userId == null)
        return false;
    const user = db.prepare('SELECT username, is_restricted FROM users WHERE id = ?').get(userId);
...
export function userMustChangePassword(userId) {
    if (userId == null)
        return false;
    const user = db.prepare('SELECT must_change_password FROM users WHERE id = ?').get(userId);
    return Boolean(user?.must_change_password);
}
export function isUsernameFullAccess(username) {
...
export function isTripOwnerRestricted(tripId) {
    const creator = db
...
export function countTripsCreatedBy(userId) {
    const row = db
        .prepare(`SELECT COUNT(*) as count FROM trip_members tm
       WHERE tm.user_id = ?
