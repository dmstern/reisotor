...
export function getPushPreferences(userId) {
    const rows = db.prepare('SELECT domain, enabled FROM push_preferences WHERE user_id = ?').all(userId);
    const byDomain = new Map(rows.map((r) => [r.domain, !!r.enabled]));
    return Object.fromEntries(PUSH_PREFERENCE_DOMAINS.map((d) => [d, byDomain.get(d) ?? true]));
}
/** Upsert je übergebenem Domain-Key (Teil- oder Vollmenge von PUSH_PREFERENCE_DOMAINS). */
export function setPushPreferences(userId, prefs) {
    const stmt = db.prepare(`INSERT INTO push_preferences (user_id, domain, enabled) VALUES (?, ?, ?)
...
export async function sendPushToTripMembers(tripId, payload, domain, excludeUserId) {
    if (!pushEnabled)
        return;
...
export async function notifyTripMembers(tripId, excludeUserId, info) {
    ...
    await sendPushToTripMembers(tripId, { title, body, tripId }, info.domain, excludeUserId);
}
