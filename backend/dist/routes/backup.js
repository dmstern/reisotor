...
function isValidPayload(body) {
    ...
    if (!data || typeof data !== 'object')
        return false;
    return REQUIRED_TABLES.every((table) => Array.isArray(data[table]));
}
...
async function resolveMissingCoordinates(data) {
    async function resolveRow(row, linkKey, latKey, lngKey) {
    ...
    for (const row of data.spots) {
        if (!row.image_url && row.lat != null && row.lng != null) {
            row.image_url = tilePreviewUrl(row.lat, row.lng);
        }
    }
    await Promise.all(data.schedule_items.map((row) => resolveRow(row, 'maps_link', 'lat', 'lng')));
    await Promise.all(data.travel_items.map(async (row) => {
...
function rowsForTable(table, userId) {
    if (table === 'users') {
        return db
            .prepare(`SELECT DISTINCT users.* FROM users
         JOIN trip_members ON trip_members.user_id = users.id
         WHERE trip_members.trip_id IN (${MEMBER_TRIP_IDS_SUBQUERY})`)
            .all(userId);
    }
    if (table === 'trips') {
        return db.prepare(`SELECT * FROM trips WHERE id IN (${MEMBER_TRIP_IDS_SUBQUERY})`).all(userId);
    }
    ...
    return db.prepare(`SELECT * FROM ${table} WHERE ${filter}`).all(userId);
}
export const backupRoutes = async (app) => {
    ...
    app.post('/backup/import', async (req, reply) => {
        ...
        // gefährden.
        const allExistingTrips = db.prepare('SELECT id FROM trips').all();
        const memberTripIds = new Set(db.prepare('SELECT trip_id FROM trip_members WHERE user_id = ?').all(req.session.userId).map((r) => r.trip_id));
        if (!allExistingTrips.every((t) => memberTripIds.has(t.id))) {
            return reply.code(403).send({
                error: 'Import nicht möglich: du bist nicht Mitglied aller aktuell vorhandenen Urlaube – ein Import würde deren Daten überschreiben.',
            });
        }
        ...
        if (!payload.data.trip_members?.length) {
            ...
            payload.data.trip_members = trips.flatMap((trip) => users.map((user) => ({
                trip_id: trip.id,
                user_id: user.id,
