...
export const notesRoutes = async (app) => {
    ...
    app.post('/notes', async (req, reply) => {
        ...
        const result = db
            .prepare('INSERT INTO notes (trip_id, title, content, content_format, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?)')
    ...
    app.put('/notes/:id', async (req, reply) => {
        const existingNote = db.prepare('SELECT trip_id FROM notes WHERE id = ?').get(req.params.id);
        if (!existingNote)
