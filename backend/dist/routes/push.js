import { getPushPreferences, getVapidPublicKey, PUSH_PREFERENCE_DOMAINS, removeSubscription, saveSubscription, setPushPreferences, } from '../push.js';
...
export const pushRoutes = async (app) => {
    ...
    app.delete('/push/subscribe', async (req, reply) => {
        ...
        if (!endpoint)
            return reply.code(400).send({ error: 'endpoint erforderlich' });
    ...
    app.put('/push/preferences', async (req, reply) => {
        const body = req.body ?? {};
        const knownDomains = new Set(PUSH_PREFERENCE_DOMAINS);
        const prefs = {};
