...
function locationFromSpot(spotId) {
    if (!spotId)
        return null;
    return db.prepare('SELECT maps_link, lat, lng FROM spots WHERE id = ?').get(spotId);
}
export const scheduleRoutes = async (app) => {
    ...
    app.post('/schedule', async (req, reply) => {
        ...
        const result = db
            .prepare(`INSERT INTO schedule_items
          (trip_id, date, end_date, time, end_time, title, note, location, maps_link, lat, lng, spot_id, idea_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
            .run(trip_id, date, end_date ?? null, time ?? null, end_time ?? null, title, note ?? null, location ?? null, spot?.maps_link ?? maps_link ?? null, spot?.lat ?? lat ?? null, spot?.lng ?? lng ?? null, spot_id ?? null, idea_id ?? null);
        recordActivity(trip_id, 'schedule', result.lastInsertRowid, 'created', req.session.userId);
        reply.code(201);
        return db.prepare('SELECT * FROM schedule_items WHERE id = ?').get(result.lastInsertRowid);
    });
    app.put('/schedule/:id', async (req, reply) => {
        const existingItem = db.prepare('SELECT trip_id FROM schedule_items WHERE id = ?').get(req.params.id);
        if (!existingItem)
        ...
        const spot = locationFromSpot(spot_id);
        const result = db
            .prepare(`UPDATE schedule_items
         SET date = ?, end_date = ?, time = ?, end_time = ?, title = ?, note = ?, location = ?, maps_link = ?, lat = ?, lng = ?,
             spot_id = ?, idea_id = ?
         WHERE id = ?`)
            .run(date, end_date ?? null, time ?? null, end_time ?? null, title, note ?? null, location ?? null, spot?.maps_link ?? maps_link ?? null, spot?.lat ?? lat ?? null, spot?.lng ?? lng ?? null, spot_id ?? null, idea_id ?? null, req.params.id);
        if (result.changes === 0)
            return reply.code(404).send({ error: 'Nicht gefunden' });
