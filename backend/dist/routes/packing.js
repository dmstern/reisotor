...
function categoryRequired(tripId) {
    const trip = db.prepare('SELECT packing_category_required FROM trips WHERE id = ?').get(tripId);
...
export const packingRoutes = async (app) => {
    ...
    app.put('/packing/:id', async (req, reply) => {
        const existingItem = db.prepare('SELECT trip_id, category FROM packing_items WHERE id = ?').get(req.params.id);
        if (!existingItem)
    ...
    app.delete('/packing/:id', async (req, reply) => {
        const existingItem = db.prepare('SELECT trip_id FROM packing_items WHERE id = ?').get(req.params.id);
        if (!existingItem)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, existingItem.trip_id, req.session.userId))
            return;
