...
function syncDiaryExcursions(entryId, excursionIds) {
...
function excursionIdsFor(entryIds) {
    const map = new Map();
    if (!entryIds.length)
        return map;
...
function syncDiarySpots(entryId, spotIds) {
...
function editorIdsFor(entryIds) {
    const map = new Map();
    if (!entryIds.length)
        return map;
    const placeholders = entryIds.map(() => '?').join(',');
    const rows = db
        .prepare(`SELECT entry_id, user_id FROM diary_entry_editors WHERE entry_id IN (${placeholders}) ORDER BY edited_at ASC`)
        .all(...entryIds);
    for (const row of rows) {
        const list = map.get(row.entry_id) ?? [];
...
function recordEditor(entryId, userId) {
    db.prepare(`INSERT INTO diary_entry_editors (entry_id, user_id, edited_at) VALUES (?, ?, ?)
     ON CONFLICT(entry_id, user_id) DO UPDATE SET edited_at = excluded.edited_at`).run(entryId, userId, new Date().toISOString());
}
...
function serializeEntry(row, excursionIds, spotIds = [], editorIds = []) {
    return {
        ...row,
...
export const diaryRoutes = async (app) => {
    app.get('/diary', async (req, reply) => {
        ...
        const rows = db
            .prepare('SELECT * FROM diary_entries WHERE trip_id = ? AND deleted_at IS NULL AND (is_draft = 0 OR author_id = ?) ORDER BY date DESC, created_at DESC, id DESC')
        ...
        return rows.map((row) => serializeEntry(row, excursionIds.get(row.id) ?? [], spotIds.get(row.id) ?? [], editorIds.get(row.id) ?? []));
    });
    ...
    app.get('/diary/likes', async (req, reply) => {
        if (!req.query.trip_id)
            return reply.code(400).send({ error: 'trip_id erforderlich' });
        if (!requireTripMember(reply, req.query.trip_id, req.session.userId))
            return;
        return db
            .prepare(`SELECT diary_likes.* FROM diary_likes
         JOIN diary_entries ON diary_entries.id = diary_likes.entry_id
         WHERE diary_entries.trip_id = ?`)
            .all(req.query.trip_id);
    });
    app.get('/diary/comments', async (req, reply) => {
        if (!req.query.trip_id)
            return reply.code(400).send({ error: 'trip_id erforderlich' });
        if (!requireTripMember(reply, req.query.trip_id, req.session.userId))
            return;
        return db
            .prepare(`SELECT diary_comments.* FROM diary_comments
    ...
    app.post('/diary/images', async (req, reply) => {
    ...
    app.post('/diary', async (req, reply) => {
        ...
        const result = db
            .prepare('INSERT INTO diary_entries (trip_id, author_id, title, content, content_format, images, date, created_at, is_draft) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
            .run(trip_id, req.session.userId, title ?? null, isHtml ? sanitizeHtml(content) : content, isHtml ? 'html' : 'legacy', JSON.stringify(images ?? []), 
        ...
        date || now.slice(0, 10), now, req.body.is_draft ? 1 : 0);
        ...
        reply.code(201);
        const row = db.prepare('SELECT * FROM diary_entries WHERE id = ?').get(entryId);
    ...
    app.put('/diary/:id', async (req, reply) => {
        ...
        if (!requireTripMember(reply, entry.trip_id, req.session.userId))
            return;
        ...
        db.prepare('UPDATE diary_entries SET title = ?, content = ?, content_format = ?, images = ?, date = ?, updated_at = ?, is_draft = ? WHERE id = ?').run(title ?? null, isHtml ? sanitizeHtml(content) : content, isHtml ? 'html' : 'legacy', JSON.stringify(images ?? []), date || entry.date, now, isDraft, req.params.id);
        ...
        const row = db.prepare('SELECT * FROM diary_entries WHERE id = ?').get(req.params.id);
    ...
    app.delete('/diary/:id', async (req, reply) => {
        const entry = db.prepare('SELECT * FROM diary_entries WHERE id = ?').get(req.params.id);
        if (!entry)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, entry.trip_id, req.session.userId))
    ...
    app.post('/diary/:id/like', async (req, reply) => {
        const entry = db.prepare('SELECT id, trip_id FROM diary_entries WHERE id = ?').get(req.params.id);
        if (!entry)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, entry.trip_id, req.session.userId))
            return;
        ...
        db.prepare('INSERT INTO diary_likes (entry_id, user_id, created_at) VALUES (?, ?, ?)').run(req.params.id, req.session.userId, new Date().toISOString());
    ...
    app.post('/diary/:id/comments', async (req, reply) => {
        const entry = db.prepare('SELECT id, trip_id FROM diary_entries WHERE id = ?').get(req.params.id);
        if (!entry)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, entry.trip_id, req.session.userId))
    ...
    app.delete('/diary/comments/:id', async (req, reply) => {
        const comment = db
            .prepare(`SELECT diary_comments.id, diary_comments.author_id, diary_entries.trip_id FROM diary_comments
         JOIN diary_entries ON diary_entries.id = diary_comments.entry_id
