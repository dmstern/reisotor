...
function serializeEntry(row, excursionIds) {
    return { ...row, images: row.images ? JSON.parse(row.images) : [], excursion_ids: excursionIds };
}
export const diaryRoutes = async (app) => {
    app.get('/diary', async (req, reply) => {
        ...
        if (!requireTripMember(reply, req.query.trip_id, req.session.userId))
            return;
        const rows = db
            .prepare('SELECT * FROM diary_entries WHERE trip_id = ? AND deleted_at IS NULL ORDER BY date DESC, created_at DESC, id DESC')
    ...
    app.post('/diary', async (req, reply) => {
        ...
        const result = db
            .prepare('INSERT INTO diary_entries (trip_id, author_id, title, content, content_format, images, date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    ...
    app.put('/diary/:id', async (req, reply) => {
        ...
        const isHtml = req.body.content_format === 'html';
        const now = new Date().toISOString();
        db.prepare('UPDATE diary_entries SET title = ?, content = ?, content_format = ?, images = ?, date = ?, updated_at = ? WHERE id = ?').run(title ?? null, isHtml ? sanitizeHtml(content) : content, isHtml ? 'html' : 'legacy', JSON.stringify(images ?? []), date || entry.date, now, req.params.id);
