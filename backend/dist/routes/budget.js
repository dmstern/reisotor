...
function isBudgetVisibleTo(budget, userId) {
...
// zurückkehren.
function requireBudgetAccessIfLinked(reply, budgetId, userId) {
    if (budgetId == null)
        return true;
    const budget = db.prepare('SELECT owner_id FROM budgets WHERE id = ?').get(budgetId);
    if (!budget) {
...
export const budgetRoutes = async (app) => {
    ...
    app.post('/budget', async (req, reply) => {
        ...
        if (!requireBudgetAccessIfLinked(reply, budget_id, req.session.userId))
            return;
        const result = db
    ...
    app.put('/budget/:id', async (req, reply) => {
        const existingExpense = db.prepare('SELECT trip_id, budget_id FROM budget_items WHERE id = ?').get(req.params.id);
        if (!existingExpense)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, existingExpense.trip_id, req.session.userId))
            return;
        if (!requireBudgetAccessIfLinked(reply, existingExpense.budget_id, req.session.userId))
            return;
    ...
    app.put('/budget/budgets/:id', async (req, reply) => {
        const existingBudget = db.prepare('SELECT trip_id, owner_id FROM budgets WHERE id = ?').get(req.params.id);
        if (!existingBudget)
            return reply.code(404).send({ error: 'Nicht gefunden' });
    ...
    app.put('/budget/allocations', async (req, reply) => {
        ...
        const parentBudget = db.prepare('SELECT trip_id, owner_id FROM budgets WHERE id = ?').get(budget_id);
        if (!parentBudget)
    ...
    app.delete('/budget/allocations/:id', async (req, reply) => {
        const allocation = db
            .prepare(`SELECT budget_allocations.id, budgets.trip_id, budgets.owner_id FROM budget_allocations
