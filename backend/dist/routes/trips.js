...
import { isUserRestricted, isTripOwnerRestricted, countTripsCreatedBy, RESTRICTED_MAX_TRIPS, RESTRICTED_MAX_MEMBERS, } from '../registrationConfig.js';
...
export const tripsRoutes = async (app) => {
    ...
    app.put('/trips/:id', async (req, reply) => {
        ...
        const roundCoord = (v) => v == null ? null : Math.round(v * 10000) / 10000;
        const locationChanged = roundCoord(lat) !== roundCoord(existing.lat) || roundCoord(lng) !== roundCoord(existing.lng);
        const packingCategoryRequired = req.body.packing_category_required !== false ? 1 : 0;
        const result = db
        ...
        if (locationChanged) {
            ...
            refreshTripWeatherSnapshots(Number(req.params.id)).catch((err) => app.log.error(err, `weatherSnapshots: refresh after location change failed for trip ${req.params.id}`));
        }
    ...
    app.get('/trips/:id/weather-history', async (req, reply) => {
        if (!requireTripMember(reply, req.params.id, req.session.userId))
            return;
        return db
            .prepare('SELECT date, lat, lng, weathercode, temp_max, temp_min, precipitation_probability FROM trip_weather_snapshots WHERE trip_id = ? ORDER BY date')
            .all(req.params.id);
    });
