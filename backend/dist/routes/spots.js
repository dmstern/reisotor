...
function planBudgetExpense(tripId, existingBudgetExpenseId, body) {
    const hasAmount = body.category === 'Unterkunft' &&
        body.amount != null &&
        body.amount > 0 &&
...
export const spotsRoutes = async (app) => {
    ...
    app.put('/spots/:id', async (req, reply) => {
        const existing = db
            .prepare('SELECT id, trip_id, lat, lng, budget_expense_id FROM spots WHERE id = ?')
            .get(req.params.id);
        if (!existing)
    ...
    app.post('/spots/:id/done', async (req, reply) => {
        ...
        if (done) {
            ...
            // plan-spot).
            const hasDate = db
                .prepare(`SELECT 1 FROM schedule_items WHERE spot_id = ? AND deleted_at IS NULL
           UNION
    ...
    app.delete('/spots/:id', async (req, reply) => {
        const existing = db
            .prepare('SELECT trip_id, budget_expense_id FROM spots WHERE id = ?')
            .get(req.params.id);
        if (!existing)
            return reply.code(404).send({ error: 'Nicht gefunden' });
        if (!requireTripMember(reply, existing.trip_id, req.session.userId))
