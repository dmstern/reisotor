...
// "Auf Tour ziehen" bleibt bewusst erhalten (eine Unterkunft kann sinnvoll Station einer Tour sein).
//
...
test.describe('Unterkunft-Spot: Aktionen, die für eine Unterkunft keinen Sinn ergeben', () => {
  test('"Einplanen" und "Als gemacht markieren" fehlen, "Auf Tour ziehen" bleibt', async ({ page }) => {
    ...
    await expect(actions).toContainText('Auf Tour ziehen');
