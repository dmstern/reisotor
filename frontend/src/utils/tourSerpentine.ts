...
export function buildTourSerpentineRows(
  ...
  while (currentIndex < total) {
    ...
    for (let i = 0; i < rowSpots.length; i++) {
      ...
      cells.push({
        type: 'spot',
        key: `spot-${current.spot.id}-${current.globalIndex}`,
        spot: current.spot,
      ...
      if (i < rowSpots.length - 1) {
        ...
        cells.push({
          type: 'leg-horizontal',
          key: `leg-horiz-${current.spot.id}-${next.spot.id}-${current.globalIndex}`,
    ...
    if (end < total) {
      ...
      rowBreak = {
        key: `row-break-${lastSpot.id}-${firstNextSpot.id}-${rowIndex}`,
        fromSpot: lastSpot,
